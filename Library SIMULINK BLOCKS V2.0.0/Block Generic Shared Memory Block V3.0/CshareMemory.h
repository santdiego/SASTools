...
public:
    ...
    CshareMemory(char *cpName, unsigned long size);
    virtual ~CshareMemory();
    //Share memory functions
    bool openShareMemory();
    bool closeShareMemory();
    bool readShareMemory(void *pfData);
    bool readShareMemory(void *pfData, unsigned long size, unsigned long index=0);//read custom_size<_size
    bool writeShareMemory(void *pfData);
    bool writeShareMemory(void *pfData, unsigned long size, unsigned long index=0);//write custom_size<_size
    ...
    void* data(); //returns pointer to data

protected:
    void *pMemory;          //Pointer to Memory dir
    char cpName[40];		//Name of the Sare Memory
    unsigned long _size;           //Size of Share Memory
