...
static VIRTUAL_DEVICE_ENTRADA *pVirtualDevice_entrada[CANTIDAD_PIONEER];
static HANDLE hShare_entrada[CANTIDAD_PIONEER];
static VIRTUAL_DEVICE_SALIDA *pVirtualDevice_salida[CANTIDAD_PIONEER];
static HANDLE hShare_salida[CANTIDAD_PIONEER];

BOOL OpenShareMemory(TCHAR *cpName, int i)
{
			TCHAR nombre_entrada[256];
			TCHAR nombre_salida[256];
			if(i<0 || i>CANTIDAD_PIONEER-1){
