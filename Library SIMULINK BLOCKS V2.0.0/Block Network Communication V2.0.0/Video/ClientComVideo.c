...
#define S_FUNCTION_NAME ClientComVideo
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	ssSetInputPortWidth(S, 2, MAXNUMBYTES);
	...
	ssSetOutputPortWidth(S, 2, MAXNUMBYTES);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    UINT         *y2 = ssGetOutputPortSignal(S,1);
	unsigned char       *y3 = ssGetOutputPortSignal(S,2);
	...
	if(signalWrite!=prevSignalWrite){
		...
		for(i=0;i<sizeMemoryOut;i++){
			pfDataIn.dataMemory[i] = u3[i];
		}
		WriteShareMemory(MEMDATOSC1,&pfDataIn);

		pfRedIn.signalWrite=signalWrite;
		pfRedIn.sizeMemoryOut=sizeMemoryOut;
		NetWriteShareMemory(MEMREDC1,&pfRedIn);
	}

	prevSignalWrite=signalWrite;

	NetReadShareMemory(MEMREDC1,&pfRedOut);
	signalRead=pfRedOut.signalRead;

	if(signalRead!=prevSignalRead){
	
		ReadShareMemory(MEMDATOSC1,&pfDataOut);
    ...
    y2[0]=pfRedOut.dataMustBeReceived;
            
	for(i=0;i<pfRedOut.dataMustBeReceived;i++){
		y3[i] = pfDataOut.dataMemory[i];
	}
