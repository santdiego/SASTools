...
#define S_FUNCTION_NAME  vrepSynchronous
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    ssSetNumDiscStates(S, 0);
     ...
     if (!ssSetNumInputPorts(S, 1)) return; //Entrada de sincronizacion 
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  {   
      ...
      if(firstTime!=1){ //Verifico si es la primera vez que corro el programa
          ...
          Sleep(20000);//espero 2 seg a que se abra Vrep
      }
      ...
      if (clientID!=-1){  //Si el servidor esta conectado iniciar simulacion 
          ...
          //simxInt dummy;
          //simxGetPingTime(clientID,&dummy);
      }
      else{
...
{
    u = (const double*) ssGetInputPortSignal(S,0);
    if(u[0]==1)
        simxSynchronousTrigger(clientID);
