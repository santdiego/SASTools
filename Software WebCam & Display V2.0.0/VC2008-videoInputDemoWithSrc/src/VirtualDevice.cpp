...
BOOL CloseShareMemory()
{
	...
	if(pVirtualDevice_salida != NULL) {
		...
		if( UnmapViewOfFile(pVirtualDevice_salida) == 0){
		...
		if( CloseHandle(hShare_salida) == 0){
...
BOOL ReadShareMemory(VIRTUAL_DEVICE_SALIDA *pfData)
{
	...
	CopyMemory((VIRTUAL_DEVICE_SALIDA *)pfData, (VIRTUAL_DEVICE_SALIDA *)pVirtualDevice_salida, sizeof(VIRTUAL_DEVICE_SALIDA));
...
BOOL WriteShareMemory(LPBITMAPINFOHEADER ImagenComp, int CantBytes)
{
	...
	CopyMemory(pVirtualDevice_salida->ucImagenComp, ImagenComp, CantBytes);
	pVirtualDevice_salida->iCantBytesImagenComp=CantBytes;
