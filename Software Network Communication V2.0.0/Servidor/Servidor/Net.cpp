...
#define MAX_BUFFER_IN MAXNUMFLOAT*sizeof(float)
...
extern CHANNEL_PROPERTIES remoteChannelProp;
...
HEADER_DATA headerDataRecv, headerDataSend;
...
void recvData(SOCKET recvSocket, char *protocol, char *buffer, int sizeData){
	...
	if(strcmp(protocol,"TCP")==0){
		...
		memcpy(&headerDataRecv, &buffer[0], sizeof(HEADER_DATA));

		if(headerDataRecv.indexPacket > indexRecvPacket){
			memcpy(dataIn, &buffer[sizeof(HEADER_DATA)], sizeData);
			
			remoteTime=headerDataRecv.localTime;

			QueryPerformanceCounter(&li);
			recvTime=double(li.QuadPart)/frec;
			roundTripDelay=recvTime-headerDataRecv.remoteTime;

			indexRecvPacket=headerDataRecv.indexPacket;

			numRecvPackets++;

			numLostPackets=indexRecvPacket-numRecvPackets;

			remoteChannelProp=headerDataRecv.channelProp;
		}
		else{
			numDelPackets++;
		}
	...
	if(strcmp(protocol,"UDP")==0){
		memcpy(&headerDataRecv, &buffer[sizeof(UINT)], sizeof(HEADER_DATA));
		if(headerDataRecv.indexPacket > indexRecvPacket){
	...
	pfRedOut.remoteNumberSentPackets=remoteChannelProp.numberSentPackets;
	pfRedOut.remoteNumberRecvPackets=remoteChannelProp.numberRecvPackets;
	pfRedOut.remoteNumberLostPackets=remoteChannelProp.numberLostPackets;
	pfRedOut.remoteNumberDelPackets=remoteChannelProp.numberDelPackets;
	pfRedOut.remoteRateSentPackets=(float)remoteChannelProp.rateSentPackets;
	pfRedOut.remoteRateRecvPackets=(float)remoteChannelProp.rateRecvPackets;
	pfRedOut.remoteRateLostPackets=(float)remoteChannelProp.rateLostPackets;
	pfRedOut.remoteRateDelPackets=(float)remoteChannelProp.rateDelPackets;
	pfRedOut.remoteRoundTripDelay=(float)remoteChannelProp.roundTripDelay;

	pfRedOut.signalRead=signalRead;

	CopyMemory(pfDataOut.dataMemory,dataIn,sizeData);
...
void sendData(SOCKET sendSocket, char *protocol, char *data, int sizeData){
	...
	if(flagCompleteConnection==true){
		...
		LARGE_INTEGER li;
		QueryPerformanceCounter(&li);
		headerDataSend.indexPacket=numSentPackets+1;
		headerDataSend.localTime=double(li.QuadPart)/frec;
		headerDataSend.remoteTime=remoteTime+(headerDataSend.localTime-recvTime);
		headerDataSend.channelProp=channelProp;
						
		if(strcmp(protocol,"TCP")==0){
			buffer=(char *)malloc(sizeof(HEADER_DATA)+sizeData);
			memset(buffer, 0, sizeof(HEADER_DATA)+sizeData);
			memcpy(&buffer[0], &headerDataSend, sizeof(HEADER_DATA));
		...
		if(strcmp(protocol,"UDP")==0){
			...
			memcpy(&buffer[0], &code, sizeof(UINT));
			memcpy(&buffer[sizeof(UINT)], &headerDataSend, sizeof(HEADER_DATA));
...
void CALLBACK timerProc(UINT Dummy1, UINT Dummy2, DWORD dwUser,DWORD Dummy4, DWORD Dummy5){
	...
	if(pfRedIn.signalWrite!=prevSignalWrite){
		...
		dataOut = (char *)malloc(pfRedIn.sizeMemoryOut*sizeof(float));
		memset(dataOut,0,pfRedIn.sizeMemoryOut*sizeof(float));

		ReadShareMemory(MEMDATOSS1,&pfDataIn);

		CopyMemory(dataOut,pfDataIn.dataMemory,pfRedIn.sizeMemoryOut*sizeof(float));

		//GetDlgItemTextA(g_hDlg,IDC_EDIT1,dataOut,sizeDataOut);
		sendData(SocketArray[0], protocol, dataOut, pfRedIn.sizeMemoryOut*sizeof(float));
