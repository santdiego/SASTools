...
char inIP[16]="127.0.0.1", outIP[16]="127.0.0.1", serverIP[16]="127.0.0.1";
USHORT inPort=20000, outPort=20001, serverPort=10000;
char protocol[4]="UDP";
bool flagConnection=false;
CHANNEL_PROPERTIES channelProp;
CHANNEL_PROPERTIES remoteChannelProp;

// Forward declarations of functions included in this code module:
ATOM				MyRegisterClass(HINSTANCE hInstance);
BOOL				InitInstance(HINSTANCE, int);
LRESULT CALLBACK	WndProc(HWND, UINT, WPARAM, LPARAM);
INT_PTR CALLBACK	InitClient(HWND, UINT, WPARAM, LPARAM);
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_CLIENTE, szWindowClass, MAX_LOADSTRING);
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_CLIENTE));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_CLIENTE);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{   
		case WM_RBUTTONDOWN: 
			...
			if(flagConnection==false)
				InsertMenu(hPopMenu,0xFFFFFFFF,MF_BYPOSITION|MF_STRING,IDM_CONNECT,_T("Conectar"));
			else
				InsertMenu(hPopMenu,0xFFFFFFFF,MF_BYPOSITION|MF_STRING,IDM_DISCONNECT,_T("Desconectar"));
			InsertMenu(hPopMenu,0xFFFFFFFF,MF_SEPARATOR,ID_FILE_SEP,_T("SEP"));				
	...
	case WM_CREATE:
		...
		NetOpenPairSharedMemory(MEMREDC1);
		OpenPairSharedMemory(MEMDATOSC1);
		readFile(inIP,&inPort,outIP,&outPort,serverIP,&serverPort,protocol);
		initWinsockDll(NULL);
		break;
	...
	case WM_COMMAND:
		...
		case IDM_STATE:
			DialogBox(hInst, MAKEINTRESOURCE(IDD_STATEBOX), hWnd, State);
			break;
		...
		case IDM_EXIT:
			...
			NetClosePairSharedMemory(MEMREDC1);
			ClosePairSharedMemory(MEMDATOSC1);
...
INT_PTR CALLBACK Config(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SendMessage(GetDlgItem(hDlg,IDC_PORTSERVER),EM_SETLIMITTEXT,(WPARAM)5,NULL);
		
		SetDlgItemTextA(hDlg,IDC_IPADDRESSIN,inIP);
		SetDlgItemTextA(hDlg,IDC_IPADDRESSOUT,outIP);
		SetDlgItemTextA(hDlg,IDC_IPADDRESSSERVER,serverIP);
		SetDlgItemInt(hDlg,IDC_PORTIN,inPort,false);
		SetDlgItemInt(hDlg,IDC_PORTOUT,outPort,false);
		SetDlgItemInt(hDlg,IDC_PORTSERVER,serverPort,false);
					...
					if(changeForSet==false){
						...
						if(index>0){
							if( !isdigit(port[index-1]) ){
								...
								if(LOWORD(wParam)==IDC_PORTSERVER)	SetDlgItemTextA(hDlg,LOWORD(wParam),portSERVER_ant);
								index--;
							...
							if( isdigit(port[index-1]) ){
								...
								if(LOWORD(wParam)==IDC_PORTSERVER)	strcpy_s(portSERVER_ant,port);
							}
						}
					}
					else{
