...
BOOL ReadShareMemory(TCHAR *cpName, VIRTUAL_DEVICE_IN *pfData)
{
	...
	if(h != -1){
		CopyMemory((VIRTUAL_DEVICE_IN *)pfData, (VIRTUAL_DEVICE_IN *)pVirtualDevice_in[h], sizeof(VIRTUAL_DEVICE_IN));
	}
	else{
...
BOOL NetReadShareMemory(TCHAR *cpName, NET_VIRTUAL_DEVICE_IN *pfData)
{
	...
	if(h != -1){
		CopyMemory((NET_VIRTUAL_DEVICE_IN *)pfData, (NET_VIRTUAL_DEVICE_IN *)pNetVirtualDevice_in[h], sizeof(NET_VIRTUAL_DEVICE_IN));
	}
	else{
		MessageBox(NULL,_T("Error: Read Virtual Device CANT FIND THE SHARE MEMORY"),_T("Error"),MB_OK);
		return FALSE;
...
BOOL NetWriteShareMemory(TCHAR *cpName, NET_VIRTUAL_DEVICE_OUT *pfData)
{
	...
	if(h != -1){
		...
		CopyMemory((NET_VIRTUAL_DEVICE_OUT *)pNetVirtualDevice_out[h], (NET_VIRTUAL_DEVICE_OUT *)pfData, sizeof(NET_VIRTUAL_DEVICE_OUT));

		if( FlushViewOfFile(pNetVirtualDevice_out[h], sizeof(NET_VIRTUAL_DEVICE_OUT)) == 0){
			MessageBox(NULL,_T("Error: Write Virtual Device::FlushViewOfFile"),_T("Error"),MB_OK);
			return FALSE;
		}
